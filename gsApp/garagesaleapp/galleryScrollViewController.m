...
- (void)loadAttributesSettings{
    ...
    galleryPageControl.numberOfPages        = countPhotos; 
    galleryPageControl.hidden               = NO;
}
...
- (void)setupProductMapping:(NSString *)localResource {
    ...
    //LoadUrlResourcePath
    [self.manager loadObjectsAtResourcePath:localResource objectMapping:productMapping delegate:self];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
     galleryPageControl.currentPage = galleryScrollView.contentOffset.x / self.view.frame.size.width;
}

-(IBAction)pageControlCliked{
    CGPoint offset = CGPointMake(galleryPageControl.currentPage * self.view.frame.size.width, 0);
